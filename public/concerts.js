...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    console.log("Form Data:", { band, details, lat, long, picture });
    ...
    fetch("http://127.0.0.1:3000/add-concert", {
      ...
      .then((newConcert) => {
        console.log("New Concert Data:", newConcert);
        const name = newConcert["ชื่อคณะ"];
        const contact = newConcert["เบอร์ติดต"];
        const details = newConcert["รายละเอียด"];
        const lat = parseFloat(newConcert["lat"]);
        const long = parseFloat(newConcert["long"]);
        const picture = newConcert["picture"];
